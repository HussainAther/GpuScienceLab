...
__global__ void stepAndRenderKernel(
  ...
  // Render: map v into a nice “inferno-ish” vibe quickly
  // (simple palette: you can replace with a LUT later)
  float t = v1;
...
void SimulationCuda::setPreset(int idx) {
  // A few fun presets (feel free to tune)
...
void SimulationCuda::registerPBO(unsigned int glPbo) {
  // Register OpenGL buffer so CUDA can write to it
...
void SimulationCuda::stepAndRenderToPBO() {
  ...
  // Map PBO to get a device pointer
