...
// Minimal passthrough shader sources
...
GLView::~GLView() {
  ...
  // Tell CUDA side to release GL interop
  m_sim.shutdown();
...
void GLView::initializeGL() {
  ...
  // Init CUDA sim and register the PBO for CUDA-GL interop
...
void GLView::paintGL() {
  // Run a few sim steps per frame for richer motion
  if (m_running) {
    for (int i = 0; i < 8; i++) {
...
void GLView::createFullscreenQuad() {
  // 2 triangles covering screen: (pos.xy, uv.xy)
  float verts[] = {
    -1.f, -1.f,  0.f, 0.f,
     1.f, -1.f,  1.f, 0.f,
     1.f,  1.f,  1.f, 1.f,
